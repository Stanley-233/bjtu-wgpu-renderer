...
std::filesystem::path ShaderPaths::ShaderRoot() {
#ifdef __EMSCRIPTEN__
    return "/shader";
#elif defined(SHADER_DIR)
    return std::filesystem::path(SHADER_DIR);
#else
    return ExecutableDir() / "shader";
#endif
}

std::filesystem::path ShaderPaths::Resolve(const std::filesystem::path& relativePath) {
    return ShaderRoot() / relativePath;
}
